...
# The presentation needs the custom Beamer theme (not tracked in git).
# If it is missing, fetch it from https://github.com/javierip/custom-beamer
if [ ! -f "beamerthemeUTN-BHI.sty" ]; then
    echo "--- Custom theme not found: fetching custom-beamer ---"
    rm -rf .custom-beamer
    git clone --depth 1 https://github.com/javierip/custom-beamer.git .custom-beamer
    cp .custom-beamer/beamerthemeUTN-BHI.sty .
    cp -r .custom-beamer/theme .
    rm -rf .custom-beamer
fi
